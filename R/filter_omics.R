...
#' assays within a `MultiAssayExperiment` object. It is useful for removing
...
#' @param assays Character vector of assay names to filter. If `NULL`, all assays are filtered.
#' @param assay_name Name or index of the assay within each `SummarizedExperiment` to use.
...
#' @param min_prop Minimum proportion of samples exceeding `min_value` (used if `method = "expression"`).
#' @param verbose Whether to print messages for each assay being filtered.
#'
#' @return A filtered `MultiAssayExperiment` object with updated assays and step record.
#' @examples
#' \dontrun{
