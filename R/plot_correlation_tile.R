...
#' stored in the metadata of a `MultiAssayExperiment` object. When `feature_type = "pcs"`,
...
#' with `pmin/pmax`. It also suppresses nonsignificant tiles based on a specified
#' p-value threshold.
#'
#' @param expomicset A `MultiAssayExperiment` object containing correlation results in metadata.
#' @param feature_type Type of correlation results to plot. One of `"pcs"`, `"degs"`, `"omics"`,
#' `"factors"`, `"factor_features"`, or `"exposures"`. Must match the key used in
#' `metadata(expomicset)$correlation[[feature_type]]`.
#' @param pval_cutoff Numeric p-value cutoff below which correlations are displayed.
#' Nonsignificant tiles are rendered in the `na_color`. Default is `0.05`.
#' @param na_color Color used to represent nonsignificant or missing correlations. Default is `"grey100"`.
#' @param fill_limits Numeric vector of length 2 defining the scale limits for correlation values. Default is `c(-1, 1)`.
#' @param midpoint Numeric value for centering the fill gradient. Default is `0`.
#'
#' @return A `ggplot2` tile plot (or a combined tile + barplot if `feature_type = "pcs"`).
#'
#' @examples
#' \dontrun{
...
plot_correlation_tile <- function(
  ...
  if (is.null(correlation_list) || !feature_type %in% names(correlation_list)) {
    stop("Correlation results for feature_type '", feature_type, "' not found in metadata.")
  }

  correlation_df <- correlation_list[[feature_type]]

  if (!all(c("var1", "var2", "correlation", "p.value") %in% colnames(correlation_df))) {
    stop("Correlation data must contain columns: var1, var2, correlation, p.value.")
  }
  ...
  if (feature_type == "pcs") {
    ...
    df <- df |>
      dplyr::mutate(
        var_pc = dplyr::case_when(is_pc1 ~ var1, is_pc2 ~ var2, .default = NA_character_),
        var_exp = dplyr::case_when(is_pc1 ~ var2, is_pc2 ~ var1, .default = NA_character_),
      ...
      dplyr::mutate(
        var_pc = factor(var_pc, levels = unique(var_pc)[order(extract_pc_number(unique(var_pc)))])
      )
    ...
    df <- df |>
      ...
      dplyr::mutate(var_exp = factor(var_exp, levels = levels(bar_df$var_exp)))

    # Create heatmap
    heatmap <- ggplot2::ggplot(df, ggplot2::aes(x = var_pc, y = var_exp, fill = correlation)) +
      ggplot2::geom_tile() +
    ...
    # Create barplot
    bar_plot <- ggplot2::ggplot(bar_df, ggplot2::aes(x = n, y = var_exp)) +
      ggplot2::geom_col(fill = "grey40") +
      ggplot2::theme_classic() +
      ggplot2::labs(x = "No. of PCs",
                    y = NULL) +
      ggplot2::theme(
    ...
    p <- ggplot2::ggplot(df, ggplot2::aes(x = var2, y = var1, fill = correlation)) +
      ggplot2::geom_tile() +
      ggplot2::scale_fill_gradient2(
        low = "blue", high = "red", mid = "white",
        midpoint = midpoint, limits = fill_limits,
