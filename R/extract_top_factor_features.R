extract_top_factor_features <- function(expOmicSet, factors, method = "percentile", percentile = 0.9, threshold = 0.3) {
  ...
  loadings_df <- loadings |> 
    map2(names(loadings), \(df, name) {
      df <- as.data.frame(df)  # Ensure df is a data frame
      df$name <- name  # Add factor name as a column
      df$features <- rownames(df)  
      rownames(df) <- NULL  # Reset rownames
      return(df) 
    }) |> bind_rows() |> 
    pivot_longer(-c(features, name), names_to = "factor", values_to = "loading")
  
  # Ensure factor names are treated as characters and match MCIA format
  factors <- as.character(factors)
  loadings_df <- loadings_df |> mutate(factor = as.character(factor))
  
  # Filter for user-specified factors
  loadings_df <- loadings_df |> filter(factor %in% factors)
  ...
  if (method == "percentile") {
    ...
    factor_thresholds <- loadings_df |> 
      group_by(factor) |> 
      dplyr::summarize(threshold = quantile(abs(loading),
                                     percentile, na.rm = TRUE),
                .groups = "drop")
  } else if (method == "threshold") {
    message("Applying raw threshold-based filtering (>|", threshold, "|)...")
    factor_thresholds <- tibble(factor = unique(loadings_df$factor), threshold = threshold)
  } else {
    stop("Invalid method. Choose 'percentile' or 'threshold'.")
  }
  
  # Merge computed thresholds with loadings
  loadings_df <- left_join(loadings_df, factor_thresholds, by = "factor")
