...
# --- Exposure Category Functional Enrichment ----------
...
.summarize_exposure_enrichment <- function(enrichment_results, p_adjust_threshold = 0.05) {
  require(tidyverse)
  ...
  message("Summarizing exposure category enrichment results...")
  
  # Convert enrichment list to a tidy dataframe
  enriched_df <- names(split(enrichment_results)) |> 
    map(~ {
      category <- .x
      if (!is.null(enrichment_results[[category]]) && nrow(enrichment_results[[category]]) > 0) {
        enrichment_results[[category]] |>
          filter(p.adjust < p_adjust_threshold) |>  # Apply p.adjust threshold
          dplyr::select(ID, Description, p.adjust, geneID, assay_name) |>
    ...
    reframe(
      genes = paste(unique(geneID), collapse = ", "), 
      num_genes = n_distinct(geneID),  # Count of unique genes per assay
      min_p_adj = min(p.adjust, na.rm = TRUE),
      max_p_adj = max(p.adjust, na.rm = TRUE),
      .groups = "drop"
    ) |> 
    group_by(ID, Description, category) |> 
    mutate(n_assays = n_distinct(assay_name)) |>  # Number of assays per exposure category
    mutate(
      all_genes = paste(unique(genes), collapse = ", "), 
      shared_genes = if (dplyr::first(n_assays) > 1) {
        shared_list <- Reduce(intersect, strsplit(unique(genes), ", "))
        if (length(shared_list) == 0) NA_character_ else paste(shared_list, collapse = ", ")
      } else {
        NA_character_
  ...
  message("Exposure category enrichment summary completed.")
