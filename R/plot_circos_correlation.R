...
#'  theme_graph node_angle
#' @importFrom ggplot2 scale_fill_manual coord_fixed labs guides
#' @importFrom ggraph scale_edge_color_gradient2
...
plot_circos_correlation <- function(
  ...
  if (feature_type == "exposures") {
    ...
    if (is.null(correlation_df)) {
      stop("No exposure-exposure correlation found.
           Run `run_correlation(..., feature_type = 'exposures')` first.")
    }
  ...
  if (!is.null(annotation_colors)) {
    ...
    cat_colors <- setNames(tidy_exp_pal[seq_along(all_categories)],
                           all_categories)
  ...
  ggraph(graph, layout = "linear",
         circular = TRUE) +
    ggraph::geom_edge_arc(aes(color = edge_weight,
                              width = edge_weight)) +
    ...
    scale_edge_color_gradient2(low = low,
                               mid = mid,
                               high = high,
