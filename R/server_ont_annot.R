...
.ont_annot_build_server <- function(ontologies) {
  ...
  function(input, output, session) {
    ...
    shiny::observeEvent(input$user_df, {
      ...
      expected <- c("selected_ontology_label","selected_ontology_id",
    ...
    output$main_ui <- shiny::renderUI({
      ...
      shiny::fluidRow(
        shiny::column(4,
                      ...
                      shiny::selectizeInput("ontology_choice","Choose Term:", choices = NULL,
                      ...
                      shiny::div(style = "padding-left:10px;",
                                 shiny::tags$div(style="background:#fde0dc;padding:4px;border-radius:3px;margin-bottom:4px;",
                                                 "Missing ontology annotation"),
                                 shiny::tags$div(style="background:#fff2cc;padding:4px;border-radius:3px;margin-bottom:4px;",
                                                 "Missing category"),
                                 shiny::tags$div(style="background:#fff7e6;padding:4px;border-radius:3px;",
    ...
    output$selected_var <- shiny::renderUI({
      ...
      else shiny::HTML("<em>No row selected</em>")
    })
    ...
    output$table <- DT::renderDT({
      ...
      DT::datatable(
        ...
        DT::formatStyle('row_status', target = 'row',
    ...
    output$ontology_description <- shiny::renderUI({
      ...
      if (nrow(term) == 1) {
        ...
        if (!is.na(desc)) {
          shiny::div(style="border-left:4px solid #71255A;padding-left:10px;margin:10px 0;",
