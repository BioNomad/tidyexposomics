...
Characterizing the relationship between a multiplicity of exposures and
disease outcomes can be a daunting task. Epidemiological studies have moved towards exposure-wide 
...
layers of biological information. These layers can also be integrated to better 
...
`tidyverse` framework, with a simple, intuitive interface. Here we provide functionality to perform quality control, sample 
...
We provide functionality to either `add` results to the existing object storing the omics/exposure data or to return results directly using using `action = "get"`. We suggest adding results, given that pipeline steps are tracked and can be output to the R console, plotted as a workflow diagram, or exported to an Excel worksheet.
...
then choose your ontology and root depth level (where a lower number means a more 
general term).
...
variables in the exposure metadata. 
...
which metadata variables are our exposure variables of interest.
...
of the data. The `run_normality_check` function is used to check the normality 
of the data. 
...
Here we see a few sample outliers, and that most variation is captured 
...
components, `n_pcs`. We set `correlation_cutoff` to `0` and `pval_cutoff` to `1` to initially include all correlations.
...
To visualize the exposure correlations, we can use the `plot_circos_correlation` function. Here we will plot the 
...
and also correlated with maternal pregnancy BPA (`hs_bpa_madj_Log2`) and 
...
Here we see that most scores are associated with an increased probability 
...
abundance analysis. Here we use the `limma_trend` method, which is a popular
...
Here we use a threshold of 0.31, but this can again be adjusted as needed.
...
identifying multi-omics features that are associated with asthma status. 
...
We set the `pval_col` and `pval_thresh` to filter our association results to
...
`plot_correlation_summary` function. Here we set the mode to `summary`, which
...
have the greatest number of associations with features, which may reflect the larger number of aerosol variables.
...
the network. We could choose any of the following:
...
Additionally, we can choose to label certain nodes using the 
...
- **Mean Eigenvector Centrality**: Measures how well-connected a node is and how
...
Note that we specify a `clustering_approach`. This will cluster our 
...
testing. Here we will examine group 5, given it seems to be driven by the most 
...
asthma and correlated with exposures in the `polyatomic entity` category 
...
We provide functionality to access the underlying
data in the `MultiAssayExperiment` object and to construct tibbles for your own analysis: 

- `pivot_sample`: Pivot the sample data to a tibble with samples as rows and
exposures as columns.

- `pivot_feature`: Pivot the feature metadata to a tibble with features as 
rows and feature metadata as columns.

- `pivot_exp`: Pivot the sample and experiment assay data to a tibble with 
samples as rows and sample metadata as columns. Additionally, there will 
...
The `pivot_feature` function pivots the feature metadata to a tibble with 
features as rows and feature metadata as columns. This can be useful for
