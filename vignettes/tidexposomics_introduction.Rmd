...
# Load data
load("../data/expom_completed_pipeline.RData")
invisible(lapply(
...
expom_qc <- expom_qc |> 
  filter_sample_outliers(outliers = c("s937","s331"))
```
...
```{r,message=FALSE,warning=FALSE,fig.height=6,fig.width=10,out.width="170%"}
...
expom_da |> 
  plot_sensitivity_summary(stability_score_thresh = 12)
```
...
expom_multi |> 
  plot_circos_exposure_shared_features(geneset = "degs")
```
...
expom_enrich |> 
  plot_dotplot_enrichment(
    geneset = "deg_exp_cor",
    go_groups = c("Group_40","Group_14","Group_23","Group_32"))
```
